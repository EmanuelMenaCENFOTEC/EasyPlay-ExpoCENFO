...
#else
#include "esp_log.h"
static const char *LOG_TAG = "EasyPlay";//Linea Modificada
#endif
...
void BleCombo::begin(void)
{
	if (!isInitialized)
	{
		BLEDevice::init(String(deviceName.c_str())); // Linea Modificada
		...
		hid->manufacturer()->setValue(String(deviceManufacturer.c_str())); // Linea Modificada
